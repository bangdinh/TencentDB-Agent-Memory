# Autonomous Shared Memory Instructions for GitHub Copilot

You have access to a shared memory system via MCP tools (`tencent-memory`, Wiki ID: `wiki-9sr5qg3i`).
All AI agents (Claude, Cursor, Antigravity, Cline, Copilot) share the SAME persistent memory.
...
- **Lưu ý**: Tuyệt đối KHÔNG tìm trong các bảng session SQL local nếu chưa tra cứu `wiki_search`.
