...
You have access to the shared persistent memory system.
All AI agents (Claude, Cursor, Antigravity, Copilot, Cline) share this SAME persistent memory.
...
Khi hệ thống có 2 server MCP kết nối song song, AI phải tự động chọn đúng MCP dựa vào nội dung:
...
*(Nếu hệ thống chỉ khai báo 1 server `tencent-memory`, tự động truyền tham số `wiki_id` tương ứng với phân loại trên).*
